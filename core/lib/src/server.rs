...
use crate::tracingcontext::TracingContext;
    ...
    macro_rules! panic_info {
        ($name:expr, $e:expr) => {{
            let error_span = match $name {
                Some(name) => {
                    error_span!("handler_panic", "Handler {} panicked.", Paint::white(name))
                }
...
async fn hyper_service_fn(
    ...
    tokio::spawn(
        async move {
		    ...
		    let tc = TracingContext::new(&dummy);
		    let span = warn_span!("req",ctx = %tc);		    
                    return rocket.send_response(r, tx)
			.instrument(span)
			.await;
                }
            };
